...
## OWASP LLM Top 10 2025 — VSF Alignment

*Added in v1.3. Scope declarations refined in v1.4.*

The OWASP Top 10 for LLM Applications 2025 provides a widely adopted reference for LLM application security risks. This section maps each OWASP 2025 entry to the VSF families that cover its configuration-layer dimension, and states precisely where the VSF's scope begins and ends relative to each risk.

The VSF is scoped to the configuration layer. Not all dimensions of every OWASP risk are configuration-layer risks. Where OWASP risks extend into the model layer, infrastructure layer, or application code layer, those dimensions are outside VSF scope. This section names those boundaries explicitly.

| OWASP 2025 Entry | Description | VSF Coverage | Scope |
|---|---|---|---|
| LLM01: Prompt Injection | Adversarial content in external inputs overrides agent instructions | VSF-01, VSF-04 | Configuration layer: reasoning trace integrity, trusted/untrusted context separation, input validation settings |
| LLM02: Sensitive Information Disclosure | Credentials or PII exposed through system prompts or agent outputs | VSF-05 | Configuration layer: credentials embedded in system prompts, security controls delegated to the LLM |
| LLM03: Supply Chain | Compromised plugins, packages, or model sources | VSF-07 | Configuration layer: plugin vetting, dependency pinning, integration registry, model source verification |
| LLM04: Data & Model Poisoning | Poisoned training data or fine-tuned model backdoors | VSF-07 (configuration scope only) | Configuration layer: model version pinning, registry source verification, integrity validation settings. Model weight poisoning and backdoor triggers are outside VSF scope — see scope note below |
| LLM05: Improper Output Handling | LLM output passed unsanitised to execution contexts | VSF-05 | Configuration layer: output routing to shell, eval(), SQL, or other execution contexts without sanitisation settings. See scope note below |
| LLM06: Excessive Agency | Agents with over-permissioned tools operating beyond intended scope | VSF-05, VSF-06 | Configuration layer: tool permission scope, least privilege enforcement, human-in-the-loop controls |
| LLM07: System Prompt Leakage | Security controls or secrets embedded in exposed system prompts | VSF-05 | Configuration layer: credentials in system prompts, authorisation logic delegated to the LLM. See scope note below |
| LLM08: Vector & Embedding Weaknesses | Adversarial RAG poisoning, cross-tenant leakage, access control gaps | VSF-04 | Configuration layer: RAG access controls, vector store permission scoping, retrieval validation settings |
| LLM09: Misinformation | Hallucination guardrails absent or ineffective | VSF-02 | Configuration layer: guardrail presence, uncertainty disclosure configuration, domain restriction settings |
| LLM10: Unbounded Consumption | No inference budget controls, Denial of Wallet exposure | VSF-06 | Configuration layer: inference budget caps, per-session token limits, spending alert configuration. See scope note below |

### Scope Notes

**LLM04 — Model Weight Poisoning:** The VSF covers the configuration signals that indicate supply chain poisoning risk — model version pinning, registry source verification, integrity validation settings. The VSF does not cover poisoned training data, backdoor triggers embedded in model weights, or adversarial fine-tuning. Those risks require training-layer tooling operating on the model itself.

**LLM05 — Output Handling Scope:** The VSF covers output routing configuration — whether LLM output is configured with sanitisation controls before reaching shell execution, eval(), database query construction, or similar execution contexts. This applies where the output routing is visible in the agent configuration. XSS and CSRF from browser-rendered LLM output, and path traversal via LLM-generated file paths, are application-layer risks that live in the consuming application's rendering and file-handling code outside the agent configuration. Those risks are standard OWASP ASVS scope.

**LLM07 — System Prompt Leakage:** Per the OWASP 2025 source definition, the system prompt should not be considered a secret, and should not be used as a security control. The real risk is not the disclosure of the prompt itself — it is the underlying misconfiguration: credentials stored in the system prompt, authorisation logic delegated to the LLM rather than enforced in external systems. VSF-05 covers these configuration conditions directly. VSF-01 (reasoning trace capture) provides the defensive capability that enables detection after the fact — it is the audit mechanism, not a coverage claim for the leakage risk itself. Testing whether a system prompt can be extracted through adversarial interaction is a runtime prompt injection test, not a configuration-layer assessment.

**LLM10 — Unbounded Consumption Scope:** The VSF covers the configuration layer of LLM10 — the presence or absence of inference budget controls, token limits, and cost anomaly alerting. Model extraction via API (accumulating queries to replicate model behaviour), functional model replication through synthetic training data generation, and side-channel attacks targeting model weights or architecture are not configuration-layer risks. Those attacks operate at the API and model layer. Recommended controls for those risks: API gateway rate limiting and output obfuscation at the infrastructure layer.
...
**Versioning:**
The VSF is maintained under semantic versioning. The current version is 1.4. Version history is maintained in this repository. Breaking changes — redefinition of existing families, removal of families, fundamental scope changes — increment the major version number. Additions and clarifications increment the minor version.

**Version history:**

| Version | Date | Changes |
|---|---|---|
| 1.0 | Feb 2026 | Initial release. Six vulnerability signature families (VSF-01 through VSF-06). |
| 1.1 | Feb 2026 | Added VSF-07: Dependency Risk (Supply Chain Integrity). Updated scope section. |
| 1.2 | Mar 2026 | Added real-world case references. Refined mechanism descriptions across all families. |
| 1.3 | Mar 2026 | Added OWASP LLM Top 10 2025 alignment section. 10/10 configuration-layer coverage mapped. |
| 1.4 | Mar 2026 | Refined OWASP scope declarations. Explicit out-of-scope boundaries added for LLM04, LLM05, LLM07, LLM10. Corrected LLM07 VSF mapping: VSF-05 primary; VSF-01 role clarified as audit capability, not leakage coverage claim. |
...
**Candidates under consideration for v1.5:**
- Multi-agent orchestration integrity (inter-agent communication, sub-agent credential isolation, orchestrator context manipulation)
- Cost-runaway via model routing manipulation (adversarial routing to high-cost models in usage-billed deployments)
